...
function MeetingCompanion({ meeting, onClose }) {
  ...
  const [seoThinking, setSbaThinking] = useState(null)
  ...
  async function handleTranslate() {
    ...
    setTranslating(true)
    try {
      const res = await fetch('/api/agents/seo-engine/translate', {
        method: 'POST',
  ...
  /* ─── SEO Think ─── */
  async function handleSEOThink() {
    if (!situationText.trim()) return
    setThinking(true)
    try {
      const res = await fetch('/api/agents/seo-engine/think', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (isRecording) {
      ...
      recognition.onresult = (event) => {
        ...
        if (finalTranscript) {
          ...
          fetch(`/api/agents/seo-engine/meetings/${meeting.id}/transcript`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  async function analyzeTranscript() {
    ...
    setAnalyzing(true)
    try {
      const res = await fetch(`/api/agents/seo-engine/meetings/${meeting.id}/transcript`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function sendToCEO() {
    try {
      const res = await fetch(`/api/agents/seo-engine/meetings/${meeting.id}/handoff-to-ceo`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
      const d = await res.json()
                ...
                <div className="mt-2 space-y-2">
                  {seoThinking.analysis && (
                    <div className="p-2 bg-amber-500/5 rounded-lg border border-amber-500/20">
                      ...
                      <p className="text-xs text-foreground">{seoThinking.analysis}</p>
                    </div>
                  )}
                  {seoThinking.decision && (
                    <div className="p-2 bg-emerald-500/5 rounded-lg border border-emerald-500/20">
                      ...
                      <p className="text-xs text-foreground">{seoThinking.decision}</p>
                    </div>
                  )}
                  {seoThinking.message_to_send && (
                    <div className="p-2 bg-accent/5 rounded-lg border border-accent/20">
                      <p className="text-[10px] text-accent mb-1">💬 Message:</p>
                      <p className="text-xs text-foreground">{seoThinking.message_to_send}</p>
                    </div>
                  )}
                </div>
              )}
...
export default function SEOPage() {
  ...
  useEffect(() => {
    fetch('/api/agents/seo-engine/status')
      .then(r => r.json())
      .then(d => {
      ...
      if (mode === 'n8n') {
        ...
        if (agentId === 'seo-agent') {
          ...
          const res = await fetch('/api/agents/seo-engine/chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message: msg, session_id: 'seo_web' }),
          })
          const d = await res.json()

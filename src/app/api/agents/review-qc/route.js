...
// Review-QC — proxy to backend AI for quality checking
export async function POST(request) {
  try {
    const body = await request.json().catch(() => ({}))
    const message = (body.message || body.brief || body.task || body.content || '').toString().trim()
    const clientName = (body.client_name || body.clientName || 'Client').toString().trim()

    if (!message) {
      return NextResponse.json(
        { success: false, error: 'Content to review is required. Example: { message: "Review this blog post..." }' },
        { status: 400 }
      )
    }

    const targetUrl = `${BACKEND_URL}/api/agents/review-qc/chat`

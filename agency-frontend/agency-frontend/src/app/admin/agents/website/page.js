...
function MeetingCompanion({ meeting, onClose }) {
  ...
  async function handleSBAThink() {
    ...
    setThinking(true)
    try {
      const res = await fetch('/api/website/think', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (isRecording) {
      ...
      recognition.onresult = (event) => {
        ...
        if (finalTranscript) {
          ...
          fetch(`/api/website/meetings/${meeting.id}/transcript`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  async function analyzeTranscript() {
    ...
    setAnalyzing(true)
    try {
      const res = await fetch(`/api/website/meetings/${meeting.id}/transcript`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function sendToCEO() {
    try {
      const res = await fetch(`/api/website/meetings/${meeting.id}/handoff-to-ceo`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
      const d = await res.json()

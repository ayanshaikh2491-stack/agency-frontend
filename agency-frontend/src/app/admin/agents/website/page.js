...
const TABS = [
  ...
  { id: 'meetings', label: 'Meetings', icon: Layout },
  { id: 'finance', label: 'Finance', icon: Palette },
...
function MeetingCompanion({ meeting, onClose }) {
  ...
  // Website Thinking state
  const [situationText, setSituationText] = useState('')
  const [websiteThinking, setSbaThinking] = useState(null)
  ...
  async function handleTranslate() {
    ...
    setTranslating(true)
    try {
      const res = await fetch('/api/agents/website-builder/translate', {
        method: 'POST',
  ...
  /* ─── Website Think ─── */
  async function handleWebsiteThink() {
    if (!situationText.trim()) return
    setThinking(true)
    try {
      const res = await fetch('/api/agents/website-builder/think', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (isRecording) {
      ...
      recognition.onresult = (event) => {
        ...
        if (finalTranscript) {
          ...
          fetch(`/api/agents/website-builder/meetings/${meeting.id}/transcript`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  async function analyzeTranscript() {
    ...
    setAnalyzing(true)
    try {
      const res = await fetch(`/api/agents/website-builder/meetings/${meeting.id}/transcript`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function sendToCEO() {
    try {
      const res = await fetch(`/api/agents/website-builder/meetings/${meeting.id}/handoff-to-ceo`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
      const d = await res.json()
      if (d?.success) {
        alert('🤝 Website → CEO handoff complete! CEO will now create workspace.')
        onClose()
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-[90vw] max-w-5xl h-[85vh] bg-card border border-border rounded-xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex gap-0 min-h-0">
          ...
          {/* Middle: Audio Translator + Website Think */}
          <div className="w-[30%] border-r border-border flex flex-col">
              ...
              {audioMode && (
                <div className="mt-2 p-2 bg-accent/5 rounded-lg border border-accent/20">
                  ...
                  <p className="text-[9px] text-muted-foreground">
                    {listeningFor === 'lead' ? 'Lead English bolega → Website Hinglish mein bolega' : 'Tum Hinglish bologe → Website English mein bolega'}
                  </p>
                </div>
              )}
            ...
            <div className="p-3 flex-1 overflow-y-auto">
              <h4 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5">
                <Brain className="size-3" /> Website Soche
              </h4>
              <textarea
                value={situationText}
                onChange={e => setSituationText(e.target.value)}
                placeholder="Situation batao, Website sochega..."
              ...
              >
                ...
                {thinking ? 'Soch raha...' : 'Website se Poocho'}
              </button>
              {websiteThinking && (
                <div className="mt-2 space-y-2">
                  {websiteThinking.analysis && (
                    <div className="p-2 bg-amber-500/5 rounded-lg border border-amber-500/20">
                      ...
                      <p className="text-xs text-foreground">{websiteThinking.analysis}</p>
                    </div>
                  )}
                  {websiteThinking.decision && (
                    <div className="p-2 bg-emerald-500/5 rounded-lg border border-emerald-500/20">
                      ...
                      <p className="text-xs text-foreground">{websiteThinking.decision}</p>
                    </div>
                  )}
                  {websiteThinking.message_to_send && (
                    <div className="p-2 bg-accent/5 rounded-lg border border-accent/20">
                      <p className="text-[10px] text-accent mb-1">💬 Message:</p>
                      <p className="text-xs text-foreground">{websiteThinking.message_to_send}</p>
                    </div>
                  )}
                </div>
              )}
...
export default function WebsitePage() {
  ...
  const [activeAgent, setActiveAgent] = useState('website-agent')
  ...
  /* ─── Agent options (ONLY Website) ─── */
  const AGENTS = [
    { id: 'website-agent', name: '📊 Website Agent (OpenCode)', desc: 'Lead qualification + Pipeline + Meetings + Finance' },
  ]

  /* ─── Website Agent Status ─── */
  const [websiteStatus, setSbaStatus] = useState('checking') // checking | online | offline
  ...
  /* ─── Check Website Agent status on mount ─── */
  useEffect(() => {
    fetch('/api/agents/website-builder/status')
      .then(r => r.json())
      .then(d => {
        if (d?.success && d?.website?.status === 'running') {
          setSbaStatus('online')
          // Add welcome message from Website
          setChat([{
            role: 'assistant',
            content: `👋 **Website Agent Online!**\n\nAI Brain: OpenClaw (open-source)\n\nMain hun aapka Website Agent:\n• 🎯 Landing page design + development\n• 📊 Portfolio + e-commerce websites\n• 📅 Website redesign + optimization\n• 💰 Performance optimization (speed, SEO)\n• 📧 Mobile-responsive design\n• 🤖 Website maintenance + updates\n\nBatao, kya karna hai?`,
            agent: 'website-agent',
          }])
        } else {
  ...
  async function send() {
    ...
    try {
      if (mode === 'n8n') {
        ...
        const res = await fetch(`${n8nUrl}/webhook/website`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (agentId === 'website-agent') {
          // Website agent goes through dedicated Website API
          const res = await fetch('/api/agents/website-builder/chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message: msg, session_id: 'website_web' }),
          })
          const d = await res.json()
          const reply = d?.response || d?.message || JSON.stringify(d)
          setChat(c => [...c, { role: 'assistant', content: reply, agent: 'website-agent' }])
        } else {
      ...
      <div className="flex-1 flex flex-col border border-border rounded-lg bg-card min-w-0">
        ...
        <div className="flex items-center gap-2.5 px-4 py-3 border-b border-border shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-foreground">📊 Website Agent (OpenCode) — Website Builder</div>
            <div className="text-[11px] text-muted-foreground flex items-center gap-1.5">
              <span className={`inline-block w-1.5 h-1.5 rounded-full ${websiteStatus === 'online' ? 'bg-emerald-500' : websiteStatus === 'offline' ? 'bg-red-500' : 'bg-yellow-500'}`} />
              {websiteStatus === 'online' ? 'AI Online · OpenCode Zen' : websiteStatus === 'offline' ? 'AI Offline' : 'Checking...'}
              {mode === 'n8n' && ' · n8n mode'}
            </div>
          </div>
        ...
        <div className="border-t border-border p-3 shrink-0">
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder={mode === 'n8n' ? 'Tell Website what to do (via n8n)...' : 'Ask Website about leads, pipeline, meetings...'}
      ...
      <div className="w-72 shrink-0 hidden lg:flex flex-col gap-3">
        ...
        <div className="border border-border rounded-lg bg-card p-3.5">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className={`inline-block w-2 h-2 rounded-full ${websiteStatus === 'online' ? 'bg-emerald-500' : websiteStatus === 'offline' ? 'bg-red-500' : 'bg-yellow-500'}`} />
            <span className="text-foreground">{websiteStatus === 'online' ? 'Online' : websiteStatus === 'offline' ? 'Offline' : 'Checking...'}</span>
          </div>
          <p className="text-[10px] text-muted-foreground mt-1.5 leading-relaxed">
            {websiteStatus === 'online'
              ? 'AI Brain: OpenClaw (open-source)\nRuns on EC2 port 9001'
              : 'Website Agent is not connected to backend'}
          </p>
        </div>
        ...
        <div className="border border-border rounded-lg bg-card p-3.5">
          ...
          <div className="space-y-1">
            {[
              ...
              { label: 'View Pipeline', icon: Globe },
              { label: 'Schedule Meeting', icon: Layout },
  ...
  const renderSettings = () => (
    <div className="flex-1 overflow-y-auto space-y-5 max-w-2xl">
      <div className="border border-border bg-card p-4">
        <h3 className="text-sm font-semibold text-foreground mb-1">Website Mode</h3>
        <p className="text-xs text-muted-foreground mb-3">Choose how Website processes your requests.</p>
        <div className="flex items-center gap-3">
          <button
            onClick={() => setMode('ai')}
  ...
  /* ═══════════════════════════════════════════════
     Meetings tab — Layout from Sales Agent
     ═══════════════════════════════════════════════ */
        ...
        <div className="border border-border rounded-lg bg-card">
          ...
          {meetingsList.length === 0 ? (
            <div className="text-center py-12 text-sm text-muted-foreground">
              <Layout className="size-8 mx-auto mb-3 text-muted-foreground/40" />
              <p>No meetings yet</p>
            </div>
          ) : (
            <div className="divide-y divide-border/60">
  ...
  const renderFinance = () => {
    ...
    return (
      <div className="flex-1 overflow-y-auto space-y-4">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
          {[
            { label: 'Pipeline Value', value: `₹${(pipelineValue || 0).toLocaleString()}`, icon: Palette, color: '#10b981' },
